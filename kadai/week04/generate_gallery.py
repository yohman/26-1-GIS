#!/usr/bin/env python
"""
Week 04 Gallery Generator
Fixes file references and generates gallery with navigation

Usage: Run this script from the week04 folder whenever new files are added
...
for row in ws.iter_rows(min_row=2, values_only=True):  # Skip header
    ...
    # Extract name without kana - split by Japanese opening parenthesis
    if full_name:
...
for student_id, student_files in sorted(students.items()):
    ...
    # Get student info
    info = student_info.get(student_id, {'name': f'Student {student_id}', 'date': 'No date', 'padlet': None})
